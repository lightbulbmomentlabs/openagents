...
impl PaneRenderer {
    ...
    pub fn paint(
        ...
        if dim_inactive_panes {
            ...
            let x0 = cutout.origin.x.clamp(canvas_bounds.origin.x, canvas_bounds.max_x());
            let x1 = cutout.max_x().clamp(canvas_bounds.origin.x, canvas_bounds.max_x());
            let y0 = cutout.origin.y.clamp(canvas_bounds.origin.y, canvas_bounds.max_y());
            let y1 = cutout.max_y().clamp(canvas_bounds.origin.y, canvas_bounds.max_y());

            if x1 <= x0 || y1 <= y0 {
                paint.scene.draw_quad(Quad::new(canvas_bounds).with_background(overlay));
                return next_layer;
            }
...
fn paint_job_inbox_pane(
    ...
    paint_primary_button(accept_bounds, "Accept selected", paint);
